...
export default function FinanceDashboard() {
  ...
  const cards = [
    { label: 'Expected Fees', value: money(data.expected), icon: <CreditCard size={21} />, tone: 'sky' },
    { label: 'Collected', value: money(data.paid), icon: <Banknote size={21} />, tone: 'green' },
    { label: 'Outstanding', value: money(data.outstanding), icon: <Users size={21} />, tone: 'amber' },
    { label: 'Net Position', value: money(data.net), icon: <TrendingDown size={21} />, tone: 'violet' },
  ];
  ...
  return (
    <div className="workspace-page">
      ...
      <div className="admin-dash-grid">
        {cards.map((card) => <div key={card.label} className={`stat-card tone-${card.tone}`}><div className="stat-top"><div className="stat-value stat-money">{card.value}</div><div className="stat-chip">{card.icon}</div></div><div className="stat-label">{card.label}</div></div>)}
      </div>
      <div className="dash-row">
        <div className="dash-panel">
          ...
          <div className="table-scroll"><table className="admin-table compact-table"><thead><tr><th>Intake</th><th>Students</th><th>Expected</th><th>Paid</th><th>Outstanding</th></tr></thead><tbody>
            {(data.byIntake || []).map((intake) => <tr key={intake._id}><td><strong>{intake.title}</strong><small className="table-subtext">{intake.program}</small></td><td>{intake.studentCount}</td><td>{money(intake.expected)}</td><td>{money(intake.paid)}</td><td className={intake.outstanding > 0 ? 'text-danger' : 'text-success'}>{money(intake.outstanding)}</td></tr>)}
            {!data.byIntake?.length && <tr><td colSpan={5} className="table-empty">No intakes configured.</td></tr>}
          </tbody></table></div>
        </div>
